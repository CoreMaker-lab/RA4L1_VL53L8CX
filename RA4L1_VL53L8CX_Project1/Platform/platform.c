		...
		uint8_t *p_value)
{
    ...
    status = R_IIC_MASTER_Read(&g_i2c_master0_ctrl, data_read, 1, false);
    assert(FSP_SUCCESS == status);
		...
		uint8_t value)
{
    ...
    status = R_IIC_MASTER_Write(&g_i2c_master0_ctrl, data_write, 3, false);
    assert(FSP_SUCCESS == status);
    /* Since there is nothing else to do, block until Callback triggers*/
    //while ((I2C_MASTER_EVENT_TX_COMPLETE != i2c_event) && timeout_ms)
    while ((I2C_MASTER_EVENT_TX_COMPLETE != i2c_event) && timeout_ms > 0)
		...
		uint32_t size)
{
    ...
    /* 向I²C设备写入寄存器地址和数据 */
    status = R_IIC_MASTER_Write(&g_i2c_master0_ctrl, data_write, (uint32_t)(2 + size), true);
    assert(FSP_SUCCESS == status);
		...
		uint32_t size)
{
    ...
    /* 写入寄存器地址（但不停止I²C总线）*/
    status = R_IIC_MASTER_Write(&g_i2c_master0_ctrl, data_write, 2, false);
    assert(FSP_SUCCESS == status);
    ...
    /* 读取数据 */
    status = R_IIC_MASTER_Read(&g_i2c_master0_ctrl, p_values, size, true);
    assert(FSP_SUCCESS == status);
